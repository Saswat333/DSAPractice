...
Initialization: Set left and right pointers to 0, product to 1 (initial empty subarray), and count to 0 (to store final
subarray count).
Sliding Window:
...
(right - left): The number of contiguous subarrays within the current window (excluding elements already removed in the
inner loop).
...
Average Case: O(n) due to the efficient sliding window approach. In each iteration, either right is incremented
(constant time) or left is incremented (constant time) in the inner loop. This leads to a maximum of n iterations overall.
Worst Case: O(2*n) if elements in nums are very close to 1 and k is just slightly larger than the product of all
elements. In this scenario, the window might shrink very slowly, leading to more iterations in the worst case.
